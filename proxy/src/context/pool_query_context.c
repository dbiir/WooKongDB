...
void pool_where_to_send(POOL_QUERY_CONTEXT *query_context, char *query, Node *node)
{
	...
	else if (REPLICATION)
	{
		if (exp_transaction)
		{
			if (is_commit(node, query))
			{
				pool_set_node_to_be_sent(query_context, exp_index);
				exp_transaction = 0;
			}
			else
			{
		...
		else if (is_begin(node, query))
		{
			int send_select_id = random() < RAND_MAX / 2? 1:2;
			exp_transaction = 1;
			exp_index = send_select_id;
			pool_set_node_to_be_sent(query_context, send_select_id);
		}
			...
			MAJOR(backend) == PROTO_MAJOR_V3)
		{
			...
			{
				/* load balance */
				double r = (((double)random()) / RAND_MAX) * (NUM_BACKENDS - 1) + 1;
				int send_select_id = (int)r;
				while (send_select_id >= NUM_BACKENDS)
				{
					r = (((double)random()) / RAND_MAX) * (NUM_BACKENDS - 1) + 1;
					send_select_id = (int)r;
				}
				pool_set_node_to_be_sent(query_context, send_select_id);
			}
			else
		...
		else
		{
			...
			else if (is_dml_query(node, query))
			{
				double r = (((double)random()) / RAND_MAX) * (NUM_BACKENDS - 1) + 1;
				int send_select_id = (int)r;
				while (send_select_id >= NUM_BACKENDS)
			...
			else if (is_ddl_query(node, query))
			{
				pool_set_node_to_be_sent(query_context, 0);
			}
			else
			{
				/* send to all nodes */

...
bool is_dml_query(Node *node,char * sql)
{
	if(node == NULL)
		return false;
	if (sql == NULL)
	...
	if(IsA(node, UpdateStmt))
		return true;
	if(IsA(node, DeleteStmt))
		return true;
	return false;
}

static char *sql_to_upper(char *str)
{
	char *p = str;
	while(*str)
	{
...
bool is_ddl_query(Node *node, char *sql)
{
	if(node == NULL)
		return false;
	else if (sql == NULL)
	...
	{
		if(strstr(sql_to_upper(sql),"CREATE") != NULL)
		{
			node->type = T_CreateStmt;
			return true;
		}
		if(strstr(sql_to_upper(sql),"ALTER") != NULL)
		{
			node->type = T_AlterTableStmt;
            return true;
		}
		if(strstr(sql_to_upper(sql),"DROP") != NULL)
        {
            node->type = T_DropStmt;
            return true;
        }
	}
	else
...
bool is_begin(Node *node, char *sql)
{
	if(strstr(sql_to_upper(sql),"BEGIN") != NULL)
    {
        return true;
    }
	else
...
bool is_commit(Node *node, char *sql)
{
	if(strstr(sql_to_upper(sql),"COMMIT") != NULL)
    {
        return true;
    }
	else
